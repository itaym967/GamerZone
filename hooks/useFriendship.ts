...
export type FriendWithProfile = Friendship & {
    friend: Pick<Profile, 'id' | 'username' | 'avatar_url' | 'is_online' | 'bio'> | null
}
...
export function useFriendship(userId: string | null | undefined) {
    ...
    const fetchFriends = useCallback(async () => {
        ...
        const pendingIn = withProfiles.filter(f => f.status === 'pending' && f.receiver_id === userId)
        const pendingOut = withProfiles.filter(f => f.status === 'pending' && f.sender_id === userId)

        setFriends(accepted)
        setPendingReceived(pendingIn)
        setPendingSent(pendingOut)
        setFriendIds(new Set(accepted.map(f => f.friend?.id).filter(Boolean) as string[]))
        setLoading(false)
    }, [userId, supabase])
        ...
        if (existing) {
            ...
            if (existing.status === 'rejected') {
                const { error } = await supabase
                    .from('friendships')
                    .update({ status: 'pending', sender_id: userId, receiver_id: targetId, updated_at: new Date().toISOString() })
                    .eq('id', existing.id)
    ...
    const getFriendshipStatus = useCallback((targetId: string): { status: FriendshipStatus; friendshipId: string | null } => {
        const asFriend = friends.find(f =>
            f.sender_id === targetId || f.receiver_id === targetId
        )
        if (asFriend) return { status: 'accepted', friendshipId: asFriend.id }

        const asSent = pendingSent.find(f => f.receiver_id === targetId)
        if (asSent) return { status: 'pending_sent', friendshipId: asSent.id }

        const asReceived = pendingReceived.find(f => f.sender_id === targetId)
        if (asReceived) return { status: 'pending_received', friendshipId: asReceived.id }

        return { status: 'none', friendshipId: null }
    }, [friends, pendingSent, pendingReceived])

    const isFriend = useCallback((targetId: string): boolean => {
