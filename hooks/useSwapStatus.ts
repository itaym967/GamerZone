...
type SwapStatus = "initial" | "pending_sent" | "pending_received" | "approved" | "rejected";
...
export function useSwapStatus(currentUserId: string | null) {
    ...
    const fetchSwapStatuses = useCallback(async (userIds: string[]) => {
    ...
    const handleRealtimeUpdate = (payload: any) => {
        ...
        const otherUserId = data.sender_id === currentUserId
            ? data.receiver_id
            : data.sender_id;
    ...
    const getSwapStatus = useCallback((userId: string): SwapStatus => {
...
function determineStatus(data: any, currentUserId: string): SwapStatus {
    ...
    if (data.status === 'pending') {
        return data.sender_id === currentUserId ? 'pending_sent' : 'pending_received';
    }
    return 'initial';
}
