...
export async function POST(request: Request) {
    try {
        ...
        if (party.leader_id !== user.id) {
            return NextResponse.json({ error: 'Only the party leader can close the party' }, { status: 403 })
        }

        const memberIds = party.party_members
        ...
        if (deleteError) {
            ...
            return NextResponse.json({ error: 'Failed to close party' }, { status: 500 })
        }
