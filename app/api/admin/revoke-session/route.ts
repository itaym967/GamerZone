...
export async function POST(request: Request) {
    try {
        ...
        if (error) {
            ...
            return NextResponse.json({ error: 'Failed to revoke session', details: error.message }, { status: 500 });
        }

        console.log(`Successfully revoked all sessions for user: ${userId}`);
        return NextResponse.json({ success: true, message: 'User session revoked' });
