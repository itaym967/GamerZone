  ...
  useEffect(() => {
    async function init() {
      ...
      if (words) {
        setUiState((prev) => ({
          ...prev,
          blockedWords: words.map((w) => w.word),
        }));
      }
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (activeChat.id === GAMERBOT_ID) {
      await sendGamerBotMessage({
        ...
        userId: user?.id,
        setInput,
        setBotMessages: (updater) =>
          setUiState((prev) => ({
            ...prev,
            botMessages:
              typeof updater === "function"
                ? updater(prev.botMessages)
                : updater,
          })),
        setIsBotTyping: (value) =>
          setUiState((prev) => ({ ...prev, isBotTyping: value })),
      });
      return;
    }
  ...
  return (
    <LazyMotion features={domAnimation}>
      <div className="flex min-h-screen bg-primary-foreground pb-24 md:pr-64 md:pb-0">
          ...
          >
            ...
            >
              <AnimatePresence initial={false}>
                ...
                {(isTyping || uiState.isBotTyping) && activeChat && (
                  <m.div
                    animate={{ opacity: 1 }}
                  ...
                  >
                    ...
                    >
                      <span
                        className={`${getTypingDotClasses(uiState.isBotTyping)} [animation-delay:-0.3s]`}
                      />
                      <span
                        className={`${getTypingDotClasses(uiState.isBotTyping)} [animation-delay:-0.15s]`}
                      />
                      <span
                        className={getTypingDotClasses(uiState.isBotTyping)}
                      />
                    </div>
                  </m.div>
                )}
        ...
        {/* Report Modal */}
        <ReportMessageModal
          isOpen={uiState.reportModal.open}
          messageId={uiState.reportModal.messageId}
          onClose={() =>
            setUiState((prev) => ({
              ...prev,
              reportModal: { open: false, messageId: null, userId: null },
            }))
          }
          reportedUserId={uiState.reportModal.userId}
          reporterId={user?.id || ""}
        />
      </div>
    </LazyMotion>
