...
export function useProfileData() {
    ...
    useEffect(() => {
        ...
        const changed =
            ...
            JSON.stringify(formData.hiddenTags) !== JSON.stringify(originalData.hiddenTags) ||
    ...
    useEffect(() => {
        ...
        const fetchAll = async () => {
            try {
                const [profileRes, tagsRes, swapsSentRes, swapsReceivedRes, swapsApprovedRes, friendsRes] = await Promise.all([
                    supabase.from('profiles').select('*').eq('id', userId).single(),
                    supabase.from('gamertags').select('*').eq('user_id', userId),
                    supabase.from('swap_requests').select('id', { count: 'exact', head: true }).eq('sender_id', userId),
                    supabase.from('swap_requests').select('id', { count: 'exact', head: true }).eq('receiver_id', userId),
                    supabase.from('swap_requests').select('id', { count: 'exact', head: true }).or(`sender_id.eq.${userId},receiver_id.eq.${userId}`).eq('status', 'approved'),
    ...
    const addGamertag = useCallback(async (platform: string, tag: string) => {
        if (!userId) return;
    ...
    const removeGamertag = useCallback(async (platform: string) => {
        if (!userId) return;
        ...
        try {
            ...
            setStats(prev => ({ ...prev, gamesCount: Math.max(0, prev.gamesCount - 1) }));
