...
const getClosePartyErrorDescription = (data: unknown) => {
  if (!(data && typeof data === "object" && "error" in data)) {
    return "שגיאה בסגירת הקבוצה";
  }
...
function PartyDetailsPageContent() {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "DELETE") {
            ...
            window.location.assign("/party-finder");
  ...
  }, [partyId, fetchPartyDetails, supabase]);
  ...
  const handleKickMember = async (userId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        let errorMessage = "שגיאה בהוצאת חבר מהקבוצה";
        if (data) {
          const maybeError = data.error;
          if (typeof maybeError === "string") {
            errorMessage = maybeError;
  ...
  const closeParty = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const description = getClosePartyErrorDescription(data);
